...
app.get('/editproduct/:id', (req, res) => {
    if (req.isAuthenticated()) {
        ...
        res.render('product.ejs', { posturl: '/editproduct/' + req.params.id, product: product })
    } else {
        res.send('<h1>You are not authenticated</h1><p><a href="/login">Login her</a></p>');
    }
})
...
//angiver productdata og categorylist
        ...
        // Man redirectes til "index.ejs" hvis man er blevet godkendt til at komme ind på siden
    } else {
        res.send('<h1>You are not authenticated</h1><p><a href="/login">Login her</a></p>');
    }
    // Denne besked sendes hvis man ikke er authoriseret til at komme ind på siden
...
app.get('/deleteuser', (req, res) => {
    //her tjekker jeg om hvorvidt at den person der trykker på knappen er authoriseret til at være på siden
    if (req.isAuthenticated()) {
